...
class BackgroundCapability(Capability):
    ...
    def run(self, command: str) -> str:
        ...
        self.engine.events.emit("background.started", task_id=task_id)
    ...
    def _execute(self, task_id: str, command: str) -> None:
        try:
            completed = subprocess.run(
                command,
                shell=True,
                cwd=self.engine.session.workspace_root,
                capture_output=True,
                text=True,
                timeout=300,
            )
        ...
        self.engine.events.emit(f"background.{status}", task_id=task_id, output=output[:1000])
