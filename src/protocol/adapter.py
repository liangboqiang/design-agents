...
class ProtocolAdapter:
    ...
    def _agent_fields(self, node: WikiNode, diagnostics: list[ProtocolDiagnostic]) -> dict[str, Any]:
        root = str(node.runtime_block.get("root_skill") or "").strip()
        if not root:
            links = [*self._links(node, "root"), *self._links(node, "links")]
            root = next((link for link in links if link.startswith("skill/")), "")
        if not root:
            diagnostics.append(ProtocolDiagnostic("error", node.node_id, "Agent Wiki Page cannot infer root_skill.", "Add a Root or Runtime root_skill field.", "root_skill"))
        toolboxes = self._runtime_list(node, "toolboxes") or self._code_items(node, "tools")
        policy = self._kv_section(node, "policy")
    ...
    def _skill_fields(self, node: WikiNode, diagnostics: list[ProtocolDiagnostic]) -> dict[str, Any]:
        child_skills = [link for link in self._links(node, "children") if link.startswith("skill/")]
        refs = [link for link in self._links(node, "links") if link.startswith("skill/")]
    ...
    def _default_permission(path: str, tool_id: str) -> int:
        ...
        if category == "workflow":
            return 2
        if any(tool_id.startswith(prefix) for prefix in ("shell.", "workspace.run", "wiki_admin.")):
            return 4
        return 1

    @staticmethod
    def _is_high_risk(tool_id: str, path: str) -> bool:
        return any(tool_id.startswith(prefix) for prefix in ("shell.", "workspace.run", "wiki_admin.", "background.run"))
