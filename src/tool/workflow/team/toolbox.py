        ...
        return [
            ToolSpec(
                ...
                lambda args: self.spawn_worker(
                    ...
                    args["skill"],
                    args["prompt"],
                    [str(item) for item in args.get("tools") or []] if args.get("tools") is not None else [str(item) for item in args.get("enhancements") or []] or list(self.runtime.tool_names),
                ),
                self.toolbox_name,
            ),
            ToolSpec("team.list_workers", "List workers", "List registered workers.", {"type": "object", "properties": {}}, lambda args: json.dumps(self._roster(), ensure_ascii=False, indent=2), self.toolbox_name),
            ToolSpec(
                "team.send_message",
                "Send worker message",
            ...
            ToolSpec("team.read_inbox", "Read lead inbox", "Read the lead inbox.", {"type": "object", "properties": {}}, lambda args: self.read_inbox("lead"), self.toolbox_name),
            ToolSpec(
                "team.broadcast",
                "Broadcast message",
