...
class BackgroundCapability(StatefulToolbox):
    ...
    def tool_specs(self):
        return [
            ToolSpec("background.run", "Run background command", "Run a shell command in the background without blocking the current chat.", {"type": "object", "properties": {"command": {"type": "string"}}, "required": ["command"]}, lambda args: self.run(args["command"]), self.toolbox_name),
            ToolSpec("background.check", "Check background task", "Check the status of a background task.", {"type": "object", "properties": {"task_id": {"type": "string"}}, "required": ["task_id"]}, lambda args: self.check(args["task_id"]), self.toolbox_name),
        ]
